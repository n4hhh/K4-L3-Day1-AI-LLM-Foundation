...
OPENAI_MODEL = os.getenv("LAB_MODEL", "Qwen/Qwen3-32B-TEE")
OPENAI_MINI_MODEL = os.getenv("LAB_MINI_MODEL", "unsloth/Mistral-Nemo-Instruct-2407-TEE")
...
def compare_models(prompt: str) -> dict:
    """
    Gọi cả 2 model với cùng prompt, trả về dict gồm:
        - "prompt": str
        - "gpt4o_response": str
        - "gpt4o_latency": float
        - "mini_response": str
    ...
    output_tokens = len(gpt4o_response) / 4
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    pricing = PRICING_PER_1K_TOKENS.get(
        model,
        PRICING_PER_1K_TOKENS.get("Qwen/Qwen3-32B-TEE")
    )
    input_cost = (input_tokens / 1000) * pricing["input"]
